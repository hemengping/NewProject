    ...
    for rect in locs:
        for teNum, tepath in templates.items():
            ...
            if np.size(loc) > 0:
                ...
                print(tepath)
                locs_result.append(
                    ((rect[0] + rect[2] / 2) / np.shape(img_rgb)[1], (rect[1] + rect[3] / 2) / np.shape(img_rgb)[0]))
                print(((rect[0] + rect[2] / 2) / np.shape(img_rgb)[1], (rect[1] + rect[3] / 2) / np.shape(img_rgb)[0]))
...
def key_click(driver,key_value):
    screen_width = driver.get_window_size()['width']
    screen_height = driver.get_window_size()['height']
    label_abc = (int(screen_width * 0.15), int(screen_height * 0.99999))
    label_1 = (int(screen_width * 0.15), int(screen_height * 0.82))
    label_123 = (int(screen_width * 0.12), int(screen_height * 0.99999))
    label_a = (int(screen_width * 0.12), int(screen_height * 0.87))
    driver.tap(positions=[label_abc])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_a])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_a])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_a])
    ...
    driver.tap(positions=[label_a])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_123])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_1])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_1])
    driver.implicitly_wait(0.1)
    driver.tap(positions=[label_1])
